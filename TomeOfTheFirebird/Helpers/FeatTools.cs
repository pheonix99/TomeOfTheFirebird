...
namespace TomeOfTheFirebird.Helpers
{
    static class FeatTools
    {
        ...
        public static void AddAsFeat(params BlueprintFeature[] features)
        {
            ...
            {
                FeatureSelectionConfigurator.For(Selections.BasicFeatSelection.AssetGuidThreadSafe).AddToFeatures(feature.AssetGuidThreadSafe);
                FeatureSelectionConfigurator.For(Selections.ExtraFeatMythicFeat.AssetGuidThreadSafe).AddToFeatures(feature.AssetGuidThreadSafe);
                FeatureSelectionConfigurator.For(Selections.BasicFeatSelection.AssetGuidThreadSafe).AddToFeatures(feature.AssetGuidThreadSafe);
                foreach(BlueprintFeatureSelection f in Selections.FeatSelections.Where(selection => feature.HasGroup(selection.Group) || feature.HasGroup(selection.Group2)))
                {
                    FeatureSelectionConfigurator.For(f.AssetGuidThreadSafe).AddToFeatures(feature.AssetGuidThreadSafe).Configure();
                }
