...
namespace TomeOfTheFirebird.New_Spells
{
    static class KeenEdge
    {
        ...
        public static BlueprintAbility BuildSPell()
        {
            ...
            var maker = SpellMakerTools.MakeSpell("KeenEdge", "Keen Edge",desc , keenSprite, Kingmaker.Blueprints.Classes.Spells.SpellSchool.Transmutation);
            ...
            IEnumerable<AbilityConfigurator> allThree = new AbilityConfigurator[] { maker, maker, offhandMaker };
           
            allThree.ApplyToAll(x => x.SetRange(AbilityRange.Close).AllowTargeting(false, false, true, true).SetAnimationStyle(Kingmaker.Visual.Animation.Kingmaker.Actions.UnitAnimationActionCastSpell.CastAnimationStyle.Directional).SetActionType(Kingmaker.UnitLogic.Commands.Base.UnitCommand.CommandType.Standard).SetMetamagics(Metamagic.Quicken, Metamagic.Extend, Metamagic.Heighten, Metamagic.Reach, Metamagic.CompletelyNormal));
            allThree.ApplyToAll(x => x.SetDurationText(LocalizedStrings.TenMinutePerLevelDuration));
        ...
        {
            var maker = SpellMakerTools.MakeSpell(main ? "KeenEdgePrimary" : "KeenEdgeSecondary", main ? "Keen Edge (Main Hand)" : "Keen Edge (Off Hand)", desc, keenSprite, Kingmaker.Blueprints.Classes.Spells.SpellSchool.Transmutation);
            string keen = "102a9c8c9b7a75e4fb5844e79deaf4c0";
            var buff = SpellMakerTools.MakeBuff(main ? "KeenEdgePrimaryBuff" : "KeenEdgeSecondaryBuff", main ? "Keen Edge (Main Hand)" : "Keen Edge(Off Hand)", desc, keenSprite);
            buff.AddBuffEnchantWornItem(keen, false, main ? Kingmaker.UI.GenericSlot.EquipSlotBase.SlotType.PrimaryHand : Kingmaker.UI.GenericSlot.EquipSlotBase.SlotType.SecondaryHand);


            var buffBuild = buff.Configure();
            var actions = ActionsBuilder.New().ApplyBuff(buffBuild.AssetGuidThreadSafe, duration: SpellMakerTools.GetContextDurationValue(Kingmaker.UnitLogic.Mechanics.DurationRate.TenMinutes, true), isFromSpell: true);
